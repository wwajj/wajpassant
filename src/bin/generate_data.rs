...
use wajpassant::board::{Board, PieceType, Color};
...
fn main() {
    ...
    let out_file = Arc::new(Mutex::new(OpenOptions::new()
        .create(true)
        .append(true)
        .open(dataset_path)
        .unwrap()));
    ...
    (0..games_to_play).into_par_iter().for_each(|game| {
        ...
        for ply in 0..200 {
            ...
            if current_eval > 1000 {
                game_result = if board.side_to_move == Color::White { 1.0 } else { 0.0 };
                break;
            } else if current_eval < -1000 {
                game_result = if board.side_to_move == Color::White { 0.0 } else { 1.0 };
                break;
            }

            let abort = Arc::new(AtomicBool::new(false));
            let best_move = search_best_move(board.clone(), 4, abort, None, false);
